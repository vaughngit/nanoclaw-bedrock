...
must_haves:
  truths:
    ...
    - "Non-main groups have tool availability restricted to security.allowedTools when configured"
...
// Setting sources: non-main groups skip 'user' to prevent shared ~/.claude permission leaks
const settingSources: ('project' | 'user')[] =
  isMain ? ['project', 'user'] :
  NANOCLAW_MODE === 'host' ? ['project'] : ['project'];

// Build tool list for non-main groups
// NanoClaw MCP tools always included (needed for IPC communication)
const nonMainTools = securityConfig?.allowedTools
  ? [...securityConfig.allowedTools, 'mcp__nanoclaw__*']
  : [
...
const queryOptions = {
  ...
  // TOOLS: main gets all tools (allowedTools for auto-approval with bypassPermissions)
  // Non-main gets tools (positive allowlist) restricting available tools
  ...(isMain
    ? {
        allowedTools: [
          'Bash', 'Read', 'Write', 'Edit', 'Glob', 'Grep',
...
if (!input.isMain && input.security) {
  log(`Security config: sandbox=${input.security.sandbox}, tools=${input.security.allowedTools ? input.security.allowedTools.join(',') : 'all'}`);
}
if (!input.isMain) {
...
Run `npm run build:agent` to compile the agent-runner. Check there are no TypeScript errors. The compiled output at `container/agent-runner/dist/index.js` should exist.
  ...
  <done>
Agent-runner uses `tools` (not `allowedTools`) for non-main groups. Non-main groups get `permissionMode: 'default'`, sandbox enabled by default, `settingSources: ['project']`, and a PreToolUse hook for permission denial messaging. Main group keeps `bypassPermissions`, no sandbox, all tools via `allowedTools`. NanoClaw MCP tools always included.
  </done>
</task>
...
3. Code review: main group has `permissionMode: 'bypassPermissions'`, `allowedTools: [...]`, and no sandbox
...
- Main: no sandbox, bypassPermissions, all tools, project+user settings
