...
<task type="auto">
  ...
  <action>
**src/index.ts -- Startup validation and banner (in `main()`):**

1. After `loadState()` and the execution mode log line, add a startup validation + banner block. This replaces the simple `config.executionMode` check with a per-group-aware check:
...
2. Replace the existing container system check block:
```typescript
// Old:
...
Move this AFTER the validation/banner block (after loadState and the new validation code), not before initDatabase.

3. Import `resolveExecutionMode` from config-loader.ts at the top:
```typescript
import { config, resolveExecutionMode } from './config-loader.js';
```

**src/index.ts -- Per-group routing in `runAgent()`:**

Replace the global `config.executionMode === 'host'` ternary with per-group resolution:
...
The rest of the function stays the same. Just replace the condition.

**src/task-scheduler.ts -- Per-group routing in `runTask()`:**

1. Import `resolveExecutionMode` from config-loader:
```typescript
import { config, resolveExecutionMode } from './config-loader.js';
```

2. Replace the global `config.executionMode === 'host'` ternary with per-group resolution:
```typescript
const mode = resolveExecutionMode(group);
const output = mode === 'host'
  ? await runHostAgent(...)
...
**src/index.ts -- register_group IPC handler:**

Update the `register_group` case in `processTaskIpc()` to accept an optional `executionMode` field. Add it to the `data` type annotation at the top of processTaskIpc, and pass it through to `registerGroup()`:

```typescript
// In the data type annotation, add:
executionMode?: 'container' | 'host';

// In the register_group case:
...
**src/index.ts -- Mode hint in sendMessage for host-mode responses:**

In `processGroupMessages()`, after calling `runAgent()` and before sending the message, append a `[host]` tag when the group's resolved mode is host:
...
Similarly in task-scheduler.ts `runTask()`, after the agent returns a message:
```typescript
