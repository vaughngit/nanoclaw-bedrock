...
<task type="auto">
  <name>Task 1: Add agent-runner local build script and fix GroupQueue type</name>
  <files>package.json, src/group-queue.ts</files>
  <action>
Two small changes that unblock the host runner:

**1. Add `build:agent` script to root package.json:**
Add a script entry: `"build:agent": "cd container/agent-runner && npm run build"`
...
**2. Fix GroupQueue.registerProcess to accept null containerName:**
...
Also update the `GroupState` interface's `containerName` field from `string | null` to match (it's already `string | null`, so just verify).

Also update the `SchedulerDependencies` interface in `src/task-scheduler.ts` -- the `onProcess` callback signature has `containerName: string`. Change it to `containerName: string | null`.
  </action>
  <verify>
...
Run `npx tsc --noEmit` from project root -- must pass with no type errors.
  ...
  <done>
`build:agent` script exists and produces dist. GroupQueue.registerProcess accepts `string | null` for containerName. TypeScript compiles cleanly.
  </done>
</task>
